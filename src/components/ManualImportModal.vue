    ...
    <div class="modal-backdrop" @click.self="$emit('close')">
      <div class="bg-card border border-border rounded-card w-full max-w-3xl flex flex-col shadow-[0_24px_60px_rgb(0_0_0/0.55)] max-h-[85vh]" role="dialog" aria-modal="true" aria-labelledby="manual-import-title">
        ...
        <div v-if="scanning" class="flex flex-col items-center justify-center gap-3 py-16 text-muted">
          ...
          <p class="text-body">Analyse du dossier…</p>
        </div>
        ...
        <div v-else-if="!scanning && items.length === 0 && !scanError" class="flex flex-col items-center gap-3 py-16 text-center px-6">
          ...
          <button @click="scan" class="btn-secondary btn-sm mt-2"><RefreshCw :size="14" /> Relancer l'analyse</button>
        </div>
        ...
        <template v-else-if="items.length > 0 && step !== 'done'">
          ...
          <div class="flex items-center justify-between px-6 py-3 border-b border-hover shrink-0 gap-3 flex-wrap">
            <p class="text-meta text-muted">
              ...
              <span v-if="alreadyImportedCount > 0"> · <span class="text-ok">{{ plural(alreadyImportedCount, 'déjà importé', 'déjà importés') }}</span></span>
              · <span class="text-accent">{{ plural(newMatchedCount, 'nouveau', 'nouveaux') }}</span>
              <span v-if="unmatchedCount > 0"> · {{ plural(unmatchedCount, 'non associé', 'non associés') }}</span>
            </p>
            <button @click="scan" class="text-meta text-secondary hover:text-primary transition-colors flex items-center gap-1.5"><RefreshCw :size="13" /> Relancer l'analyse</button>
          </div>
          ...
          <div class="overflow-y-auto flex-1 px-6 py-3 flex flex-col gap-2">
            ...
            >
              ...
              >
                <option :value="null">Aucun épisode</option>
                <optgroup v-for="season in seasons" :key="season.id" :label="season.season_number === 0 ? 'Spéciaux' : `Saison ${season.season_number}`">
                  ...
                  >
                    ...
                    {{ isEpisodeOrganized(ep.id) ? '(déjà importé)' : '' }}
                  </option>
                </optgroup>
              </select>
            </div>
          ...
          <div class="flex items-center justify-between gap-4 px-6 py-4 border-t border-hover shrink-0 flex-wrap">
            ...
            <div class="flex gap-2.5">
              ...
              <button
                  ...
                  class="btn-primary"
              >
                {{ importing ? 'Import…' : `Importer ${plural(newMatchedCount, 'fichier')}` }}
              </button>
            </div>
          </div>
        ...
        <div v-else-if="step === 'done'" class="flex flex-col items-center gap-4 px-6 py-10 text-center overflow-y-auto">
          ...
          <div class="flex flex-col gap-1">
            ...
            <p class="text-body text-secondary">
              {{ plural(importResult.done, 'fichier importé', 'fichiers importés') }}
              <span v-if="importResult.errors.length > 0" class="text-err">
                · {{ plural(importResult.errors.length, 'erreur') }}
              </span>
            </p>
          </div>
...
interface FileItem {
  ...
  originalEpisodeId : number | null   // épisode auquel le fichier était déjà associé lors de l'analyse
  hash              : string | null
...
function onEpisodeChange(i: number) {
  ...
  // Le fichier choisi remplace ceux déjà importés pour cet épisode
  if (newEpId !== null) {
...
async function scan() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      scanError.value = data.error ?? "Impossible d'analyser le dossier."
      return
    }
    const data = await res.json()
...
async function doImport() {
  ...
  try {
    ...
    const data = await res.json()
    if (!res.ok) {
      importError.value = data.error ?? "Impossible d'importer les fichiers."
      toast(data.error ?? "Impossible d'importer les fichiers", 'error')
      return
    }
    ...
    if (data.done > 0) {
      toast(plural(data.done, 'fichier importé', 'fichiers importés'), 'success')
    }
    if ((data.errors ?? []).length > 0) {
      toast(`${plural(data.errors.length, 'fichier')} en erreur`, 'error')
    }

    // Le parent recharge les épisodes importés
    emit('imported')
...
function backToList() {
  step.value = 'list'
  scan()   // nouvelle analyse pour marquer les fichiers importés
}
