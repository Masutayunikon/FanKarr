  ...
  <div class="flex flex-col gap-6">
    ...
    <ol class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <li v-for="(item, i) in flow" :key="item.title" class="card flex gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="text-meta text-muted mt-0.5 leading-relaxed">{{ item.text }}</p>
        </div>
      </li>
    </ol>
    ...
    <div class="card flex flex-col gap-3">
      ...
      <ul class="flex flex-col gap-2">
        <li v-for="item in checklist" :key="item.label" class="flex items-start gap-2.5 text-body">
    ...
    <div v-if="isDocker" class="flex items-start gap-3 px-4 py-3 rounded-field border border-accent/30 bg-accent-muted">
      ...
      <div class="flex flex-col gap-1">
        ...
        <p class="text-meta text-muted leading-relaxed">
          ...
          et la médiathèque (ex. <span class="text-secondary">/data/downloads</span> et
          <span class="text-secondary">/data/medias</span>).
        </p>
      </div>
    </div>
