<template>
  <div class="min-h-screen bg-shell flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
      ...
      <!-- Chargement -->
      <div v-if="status === 'loading'" class="settings-card p-8 text-center text-sm text-muted">
        Vérification du lien…
      </div>

      <!-- Lien invalide -->
      <div v-else-if="status === 'invalid'" class="settings-card p-8 flex flex-col gap-3 text-center">
        <p class="text-sm font-semibold text-red-400">Lien invalide</p>
        <p class="text-sm text-muted">{{ invalidReason }}</p>
      </div>

      <!-- Succès -->
      <div v-else-if="status === 'done'" class="settings-card p-8 flex flex-col gap-4 text-center">
        <p class="text-sm font-semibold text-green-400">Compte créé !</p>
        <p class="text-sm text-muted">Tu peux maintenant te connecter.</p>
        <RouterLink to="/auth" class="btn-primary w-full py-2.5 text-center">Se connecter</RouterLink>
      </div>

      <!-- Formulaire d'inscription -->
      <div v-else class="settings-card p-8 flex flex-col gap-5">
        <div>
          <p class="text-xs text-accent font-medium mb-1">Invitation FanKarr</p>
          <h1 class="text-lg font-semibold text-primary">Créer un compte</h1>
          <p v-if="note" class="text-xs text-muted mt-1">{{ note }}</p>
        </div>

        <div class="flex flex-col gap-4">
          <div>
            <label class="settings-label mb-1.5">Identifiant</label>
            <input v-model="username" type="text" placeholder="username" class="settings-input" @keyup.enter="submit" />
          </div>
          <div>
...
onMounted(async () => {
  ...
  if (!res.ok || !data.valid) {
    invalidReason.value = data.reason ?? 'Lien invalide'
